...
use IO::File;
use MIME::Lite;
use lib "/home/fcs/clients/yrocher/web/params/";
# permet d'ajouter des repertoires à @INC
use lib "/home/fcs/clients/yrocher/web/lib/";
# permet d'ajouter des repertoires à @INC
...
open( FICHIER_LOG, ">> $fichier_log" )
|| die "Impossible ouvrir fichier log !\n";
my $tmp_date     = now;
...
my $dbh =
DBI->connect( $base_fcs_dsn, $base_fcs_user, $base_fcs_password, {
    AutoCommit => 1
}
);
if ( $dbh eq undef ) {
...
sub traiteYr1() {
    ...
    while (<PIPE_LS>) {
        ...
        while (<FTP_LOG>) {
            ...
            my @res_split =
            split( /\s+/, $xtemp );
            # $res_split[13] contient le login
            if ( $res_split[13] ne $login_bld ) {
                next;
            }
            if ( "/" . $file_in eq $res_split[8] and $res_split[17] eq "c" ) {
        ...
        if ($transfert_OK) {
            ...
            while (<FILE_IN>) {
                ...
                if ( substr( $arr_date, 0, 1 ) eq '2' ) {
                    # la date semble renseignée
                    ...
                    while ( my $data = $rs->fetchrow_hashref ) {
                        my $date_tmp = date {
                            year  => substr( $arr_date, 0, 4 ),
                            month => substr( $arr_date, 4, 2 ),
                            day   => substr( $arr_date, 6, 2 )
                        };
                        if ( $data-> {
                            'linerterm'
                        }
                        ...
                        my $date_planned =
                            $date_tmp->year
                          . sprintf( "%02d", $date_tmp->month )
                        . sprintf( "%02d", $date_tmp->day );
                        $ru->execute( $arr_date, $arr_time, $date_planned,
                        $data-> {
                            'container'
                        }
                        , $data-> {
                        ...
                        ,
                            "PLANNED DATE",
                            '[YR1]Auto Planned Date for container '
                        . $data-> {
                            'container'
                        }
                        . '.',
                ...
                if ( substr( $whse_date, 0, 1 ) eq '2' ) {
                    ...
                    while ( my $data = $rs->fetchrow_hashref ) {
                        ...
                        eq 'D' ) {
                            ...
                            ,
                                "PLANNED DATE",
                                '[YR2]Auto Planned Date for container '
                            . $data-> {
                                'container'
                            }
                            . '.',
                        ...
                        eq 'S' ) {
                            my $date_tmp = date {
                                year  => substr( $whse_date, 0, 4 ),
                                month => substr( $whse_date, 4, 2 ),
                                day   => substr( $whse_date, 6, 2 )
                            };
                            if ( $data-> {
                                'linerterm'
                            }
                            ...
                            ,
                                "PLANNED DATE",
                                'Auto Planned Date for container '
                            . $data-> {
                                'container'
                            }
                            . '(SAILING => PLANNED).',
                ...
                if ( substr( $whse_date, 0, 1 ) eq '2' ) {
                    ...
                    while ( my $data = $rs->fetchrow_hashref ) {
                        ...
                        eq 'D' ) {
                            ...
                            ,
                                "ARRIVED FND DATE",
                                'Auto Arrived FND Date for container '
                            . $data-> {
                                'container'
                            }
                            . '.',
...
sub traiteYr4() {
    ...
    my $sitmp =
" INSERT INTO stripped_tmp (container, po_root, fnd_initial_date, sku_id, pcs, ctn)
	      VALUES ( ?, ?, ?, ?, ?, ? )
    ";
...
####################################################################
    ...
    my $sidh =
" INSERT INTO delivery_header ( num_do, date_depart, heure_depart, in_road_tracing )
		VALUES ( ?, ?, ?, '1')
    ";
...
print "YR4";
				...
				print $ct;
                    ...
                    if ( substr( $whse_date, 0, 1 ) eq '2' ) {
                        ...
                        while ( my $data = $rs->fetchrow_hashref ) {
                            ...
                            ,
                                "STRIPPED DATE",
                                'Auto Stripped Date for container '
                            . $data-> {
                                'container'
                            }
                            . '.',
                ...
                if ( substr( $ligne, 0, 1 ) eq 'D' ) {
                    ...
                    if ( $rtmp->rows > 0 ) {
                        ...
                        $rutmp->execute(
                            $pcs + $data[0],
                            $ctn + $data[1],
                            $ct, $po_root, $fnd_initial_date, $id_sku
                        );
                        if ( $dbh->errstr ne undef ) {
                            print FICHIER_LOG $dbh->errstr . "\n" . $sutmp;
                            $rutmp->finish;
                        ...
                        $ritmp->execute( $ct, $po_root, $fnd_initial_date,
                        $id_sku, $pcs, $ctn );
                        if ( $dbh->errstr ne undef ) {
                            print FICHIER_LOG $dbh->errstr . "\n" . $sitmp;
                            $ritmp->finish;
                 ...
                 #		le num_do contient dans ce cas le numéro du CONTAINER
                    ...
                    # ON RECHERCHE LE CPC
                    my $scpc =
                    "SELECT TRIM(code_entrepot) FROM ref_entrepot WHERE code_wms = '$code_wms' ";
	...
	AND UPPER(SUBSTR(fss.po,1,10)) = '$po_root'
        AND TRIM(SUBSTR(tcpm.container,1,11)) = '$ct'
                    ...
                    my $bl = $rbl->fetchrow;
                    $rbl->finish;
                    $rdd->execute( $ct, $bl, $po_root, $id_sku, $pays_bld,
                    $reseau_bld );
                    if ( $dbh->errstr ne undef ) {
                        print FICHIER_LOG $dbh->errstr . "\n" . $sdd;
                        $rdd->finish;
...
# ==========================================================================
    ...
    while ( my @data = $rtmp->fetchrow ) {
        ...
        if ( $rqlr->rows == 1 ) {
            ...
            if ( $result[1] eq '0' ) {
                # SI QUANTITES STRIPPEES NON RENSEIGNEES
                $rulr->execute(
                    $stripped_sku, $stripped_ctn, $container,
                    $result[0],    $sku_id
                );
                if ( $dbh->errstr ne undef ) {
                    print FICHIER_LOG $dbh->errstr . "\n" . $sulr;
...
sub traiteYr5() {
    ...
    # =============================
    my $sh =
    " SELECT * FROM delivery_header WHERE num_do = ? AND nom_livraison =? ";
...
" INSERT INTO delivery_header ( num_do, nom_livraison, date_depart, heure_depart, date_livraison_prevue, heure_livraison_prevue, num_commande, colis_reconditionnes, date_arrivee, heure_arrivee, date_start_loading, heure_start_loading, date_end_loading, heure_end_loading ) 
 VALUES ( ?, ?, ?, ?, ?, ?, ? ,? ,? ,? ,? ,? ,? ,? )
    ...
    my $sd1 =
    " SELECT TRIM(code_entrepot) FROM ref_entrepot WHERE code_wms = ? ";
    my $rd1 = $dbh->prepare($sd1);
    my $sd2 =
      " SELECT id_header, palettes, pieces, colis, poids FROM delivery_detail d
LEFT JOIN delivery_header h
ON h.id = d.id_header
WHERE TRIM(h.num_do) = ?
...
AND reseau_bld = ?
    ...
    while (<PIPE_LS>) {
        ...
        if ($transfert_OK) {
            ...
            while (<FILE_IN>) {
                ...
                if ( substr( $ligne, 0, 1 ) eq 'E' ) {
                    ...
                    $nom_livraison =
                    &trimwhitespace( substr( $ligne, 125, 35 ) );
                    my $num_commande =
                    &trimwhitespace( substr( $ligne, 41, 10 ) );
                    # le numéro e_entrepot
                    ...
                    my $date_arrivee =
                    &trimwhitespace( substr( $ligne, 957, 8 ) );
                    my $heure_arrivee =
                    &trimwhitespace( substr( $ligne, 965, 4 ) );
                    my $date_start_loading =
                    &trimwhitespace( substr( $ligne, 969, 8 ) );
                    my $heure_start_loading =
                    &trimwhitespace( substr( $ligne, 977, 4 ) );
                    my $date_end_loading =
                    &trimwhitespace( substr( $ligne, 981, 8 ) );
                    my $heure_end_loading =
                    &trimwhitespace( substr( $ligne, 989, 4 ) );
                    my $date_depart =
                    &trimwhitespace( substr( $ligne, 109, 8 ) );
                    my $heure_depart =
                    &trimwhitespace( substr( $ligne, 953, 4 ) );
                    my $date_livraison_prevue =
                    &trimwhitespace( substr( $ligne, 117, 8 ) );
                    my $heure_livraison_prevue =
                    &trimwhitespace( substr( $ligne, 602, 4 ) );
                    my $colis_reconditionnes =
                    &trimwhitespace( substr( $ligne, 949, 4 ) );
                    print FICHIER_LOG
                    ...
                    if ( $rh->rows > 0 ) {
                        # si l'enregistrement existe déjà : ON NE FAIT RIEN
                        $rh->finish;
                        print FICHIER_LOG
                        ...
                        $rh->finish;
                        $rih->execute(
                            $num_do,                $nom_livraison,
                            $date_depart,           $heure_depart,
                            $date_livraison_prevue, $heure_livraison_prevue,
                            $num_commande,          $colis_reconditionnes,
                            $date_arrivee,          $heure_arrivee,
                            $date_start_loading,    $heure_start_loading,
                            $date_end_loading,      $heure_end_loading
                        );
                        if ( $dbh->errstr ne undef ) {
                            print FICHIER_LOG $dbh->errstr . "\n" . $sih;
                            $rih->finish;
		...
		( num_do, nom_livraison, date_depart, heure_depart, date_livraison_prevue, heure_livraison_prevue, num_commande, colis_reconditionnes, date_arrivee, heure_arrivee, date_start_loading, heure_start_loading, date_end_loading, heure_end_loading ) 
		VALUES ( 
		'$num_do', 
		'$nom_livraison', 
		'$date_depart', 
		...
		'$num_commande', 
		'$colis_reconditionnes', 
		'$date_arrivee', 
		'$heure_arrivee', 
		'$date_start_loading', '$heure_start_loading', '$date_end_loading', '$heure_end_loading' )
		;
                        ";
                        print FICHIER_LOG "\n	=> Insertion $sqlr";
                        $rih->finish;
                    ...
                    if ( substr( $ligne, 0, 1 ) eq 'D' ) {
                        ...
                        #
                        my $po_root =
                        &trimwhitespace( substr( $ligne, 26, 15 ) );
                        if ( index( $po_root, '/' ) > 0 ) {
                            $po_root =
                            substr( $po_root, 0, index( $po_root, '/' ) );
                        }
                        my $pays = &trimwhitespace( substr( $ligne, 51, 2 ) );
                        my $code_support =
                        &trimwhitespace( substr( $ligne, 167, 3 ) );
                        my $bl = &trimwhitespace( substr( $ligne, 170, 15 ) );
                        my $palettes =
                        &trimwhitespace( substr( $ligne, 55, 5 ) );
                        my $pieces =
                        &trimwhitespace( substr( $ligne, 67, 10 ) );
                        ...
                        $rd2->execute( $num_do, $nom_livraison, $bl, $po_root,
                        $sku, $pays, $code_support );
                        if ( $dbh->errstr ne undef ) {
                            print FICHIER_LOG $dbh->errstr . "\n" . $sd2;
                            $rd2->finish;
                            $dbh->disconnect;
                        ...
                        if ( $id_header eq '' ) {
                            # ON INSERT
                            $rd3->execute(
                                $num_do,       $nom_livraison, $bl,
                                $po_root,      $sku,           $pays,
                                $code_support, $cpc_fcs,       $palettes,
                                $pieces,       $colis,         $poids
                            );
                            if ( $dbh->errstr ne undef ) {
                                print FICHIER_LOG $dbh->errstr . "\n" . $sd3;
...
sub traitePdf() {
    ...
    while (<PIPE_LS>) {
        ...
        if ($transfert_OK) {
            ...
            my $num_commande = '';
            if ( substr( $res_split[0], 0, 2 ) eq 'LC' ) {
                # Liste de colisage
            ...
            if ( $num_do ne '' ) {
                ...
                if ( substr( $res_split[0], 0, 2 ) eq 'LC' ) {
                    # Liste de colisage
                    $file_out =
                        '/home/fcs/yrgroup/web/downloads/LC' 
                      . $num_do . '_'
                    . $num_commande . '.pdf';
                }
                else {
                    # Bon de livraison
                    $file_out =
                        '/home/fcs/yrgroup/web/downloads/' 
                      . $num_do . '_'
                    . $num_commande . '.pdf';
                }
